...
├── GET /api/debris/trajectory ← trajectory for one object (ECI + geodetic)
└── GET /api/debris/trajectories/all ← ground-track lat/lon for all objects
        │
        ▼  dashboard/  (React + Vite, port 5173)
Interactive Dashboard
├── Globe          — 3D WebGL Earth, debris dots, collision arcs
├── Analytics      — Model benchmarks, RMSE chart, forecast overlay
├── Collision Alerts — Sortable risk table
├── Debris Objects — Full catalog with orbital parameters
├── Trajectories   — Animated 3D ground tracks for all 200 objects (globe.gl)
│                    Regime filter chips (LEO/MEO/HEO/GEO), object detail panel
└── System Status  — Pipeline run history, health checks
```
...
| GET | `/api/debris/trajectory` | Full 72-hour ECI trajectory for one object (`?norad_id=&downsample=`) |
| GET | `/api/debris/trajectories/all` | Lightweight ground-track lat/lon for all objects (`?downsample=&regime=&limit=`) |

---

## Notebooks

Three Jupyter notebooks document the analysis and are self-contained (all data loading inline):
...
| `notebooks/trajectory_visualisation.ipynb` | **8-section interactive Plotly notebook** — 3D ECI orbit, ground tracks, altitude/speed profiles, multi-object overlays, animated trajectories, conjunction pair visualisation, and orbital regime histogram for all 200 propagated objects |
